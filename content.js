...
(function () {
  ...
  (function loadPublished() {
    ...
    // Чия версія свіжіша: опублікована на сайт чи локальна чернетка цього браузера.
    // Публікація — явна дія «зробити це живим», тому вона перемагає старі чернетки.
    // Інакше на телефоні, де колись відкривали адмінку, назавжди залишався б старий вміст.
    const pubAt = Number(published._publishedAt || 0);
    const locAt = Number(local._savedAt || 0);
    if (pubAt >= locAt) {
      saved = published;                 // опубліковане новіше — показуємо його
    } else {
