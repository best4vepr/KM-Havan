...
function PublishEditor({ c }) {
  ...
  const publish = async () => {
    ...
    const res = await ghPublish(cfg, json);
    if (res.ok) res.message += ' Зміни на сайті за 1–2 хвилини.';
    setMsg(res); setBusy('');
  };

  const box = (m) => (
    <div style={{
      marginTop: 14, padding: '12px 14px', borderRadius: 8, fontFamily: 'var(--font-sans)', fontSize: '0.875rem',
      fontWeight: 600, lineHeight: 1.5,
      background: m.ok ? 'var(--green-100)' : 'var(--red-100)',
      color: m.ok ? 'var(--green-600)' : 'var(--red-600)',
    }}>{m.message}</div>
  );
      ...
      <Panel title="Публікація прямо на сайт"
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginTop: 4 }}>
          ...
          <button onClick={publish} disabled={!!busy} style={{
            ...
            fontFamily: 'var(--font-sans)', fontSize: '0.9375rem', fontWeight: 700, color: 'var(--ink-900)',
            background: 'var(--sand-500)', border: 'none', borderRadius: 8, padding: '11px 20px', opacity: busy ? 0.6 : 1,
          }}>
...
Object.assign(window, { Panel, Row, PublishEditor, HeroEditor, AmenitiesEditor, NewQuarterEditor, SchemesEditor, InfraEditor, GalleryEditor, ContactsEditor, FilesEditor, DesignEditor });
